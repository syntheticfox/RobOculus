...
int main(){
	...
	/*
	const GLFWvidmode* mode = glfwGetVideoMode(detect_HMD(ovr));
	...
	*/
	...
	//GLFWwindow* second_window = glfwCreateWindow(1020, 960, "RobOculus", glfwGetPrimaryMonitor(), NULL);
	if (!window){
		glfwTerminate();
		exit(EXIT_FAILURE);
	}
	...
	while (!glfwWindowShouldClose(window)){
		...
		GLuint textureR = createTexture(capRight);
		capRight.release();

		ovrHmd_BeginFrame(ovr->hmd, 0);
		for (int eyeIndex = 0; eyeIndex < ovrEye_Count; eyeIndex++){
		ovrEyeType eye = ovr->hmd->EyeRenderOrder[eyeIndex];
		headPose[eye] = ovrHmd_GetHmdPosePerEye(ovr->hmd, eye);
